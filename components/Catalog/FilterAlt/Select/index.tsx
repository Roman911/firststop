...
const Select: FC<SelectProps> = (
	...
	return <div className='relative mt-2 rounded-lg border-b-1 border-gray-100 dark:bg-gray-700 dark:border-gray-700'>
		<button
			type='button'
			onClick={ () => handleClickOpen() }
			className='relative w-full cursor-default py-4 pr-10 text-left focus:outline-none pl-3'>
      <span className='flex items-center'>
        <span className='block truncate text-sm dark:text-gray-300'>{ t(label) }</span>
      </span>
			<span className="pointer-events-none absolute inset-y-0 right-0 ml-3 flex items-center pr-2.5">
        <Icons.ChevronDownIcon className='w-3 h-3 stroke-black dark:stroke-gray-300'/>
      </span>
		</button>
					...
					<div className='inline-flex items-center'>
						...
						>
							<input
								...
								id={ `${ name }-${ item.value }` }
								type='checkbox'
								className='peer relative h-7 w-7 bg-white dark:bg-gray-800 appearance-none cursor-pointer rounded-sm border border-black dark:border-gray-400 transition-all checked:border-primary checked:bg-primary dark:checked:bg-primary hover:border-primary'
							/>
							<div
								className='pointer-events-none absolute top-2/4 left-2/4 -translate-y-2/4 -translate-x-2/4 peer-checked:opacity-100'>
								<svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 14 14" fill="none"
										 className="h-3.5 w-3.5 fill-white dark:fill-gray-800">
									<path
						...
						<label className="mt-px cursor-pointer select-none font-medium ml-2.5 dark:text-white">
							{ item.label }
						</label>
					</div>
				</li>
				...
				<div className="inline-flex items-center ml-8">
					...
					>
						<input
							...
							id='studded'
							type="checkbox"
							className="peer relative h-7 w-7 bg-white dark:bg-gray-800 appearance-none cursor-pointer rounded-sm border border-gray-200 transition-all checked:border-primary checked:bg-primary dark:checked:bg-primary hover:border-primary"
						/>
						<div
